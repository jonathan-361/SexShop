...
exports.alterCartTable = async (req, res) => {
  ...
  try {
    ...
    `);
    for (const fk of fks) {
      await conn.query(`ALTER TABLE shopping_cart DROP FOREIGN KEY \`${fk.CONSTRAINT_NAME}\``);
    }
    ...
    for (const idx of idxs) {
      await conn.query(`ALTER TABLE shopping_cart DROP INDEX \`${idx.INDEX_NAME}\``);
    }

    await conn.query('ALTER TABLE shopping_cart DROP COLUMN store_id');
    await conn.query('ALTER TABLE shopping_cart ADD UNIQUE KEY uq_cart_user (user_id)');
    ...
    if (existingFk.length === 0) {
      await conn.query(`ALTER TABLE shopping_cart ADD CONSTRAINT fk_cart_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE`);
    }
...
exports.clearAll = async (req, res) => {
  ...
  try {
    ...
    const tables = [
      'platform_commissions','shipments','payments','order_items','orders',
      'shopping_cart_items','shopping_cart','product_images','products',
      'brands','categories','customer_addresses','delivery_methods',
      'store_address','stores','users',
    ];
...
exports.migrateSingleStore = async (req, res) => {
  ...
  try {
    ...
    await run('delete platform_commissions data', 'DELETE FROM platform_commissions');
    await run('drop platform_commissions', 'DROP TABLE IF EXISTS platform_commissions');
    ...
    `);
    if (prodFks.length > 0) {
      await run('drop fk_products_store', 'ALTER TABLE products DROP FOREIGN KEY fk_products_store');
    }
    // 4b. Index
    ...
    `);
    if (prodIdx.length > 0) {
      await run('drop idx_products_store', 'ALTER TABLE products DROP INDEX idx_products_store');
    }
    // 4c. Column
    ...
    `);
    if (prodCol.length > 0) {
      await run('drop products.store_id', 'ALTER TABLE products DROP COLUMN store_id');
    }
    ...
    `);
    if (ordFk.length > 0) {
      await run('drop fk_orders_store', 'ALTER TABLE orders DROP FOREIGN KEY fk_orders_store');
    }
    ...
    if (ordStoreIdx.length > 0) {
      await run('drop idx_orders_store', 'ALTER TABLE orders DROP INDEX idx_orders_store');
    }
    ...
    if (ordStoreCol.length > 0) {
      await run('drop orders.store_id', 'ALTER TABLE orders DROP COLUMN store_id');
    }
    ...
    `);
    if (commCol.length > 0) {
      await run('drop orders.commission_amount', 'ALTER TABLE orders DROP COLUMN commission_amount');
    }
