...
// Función auxiliar interna para validar el rol de admin y no repetir código
const isAdmin = (req, res) => {
  if (req.user && req.user.role === "admin") {
    res.status(403).json({
      message:
        "Acceso denegado. Los administradores no tienen carrito de compras.",
    });
    return true;
  }
  return false;
...
exports.getCart = async (req, res) => {
  if (isAdmin(req)) return; // Bloquea si es admin
...
exports.addItemToCart = async (req, res) => {
  ...
  if (!product_id || !quantity || !unit_price) {
    return res
      .status(400)
      .json({ message: "product_id, quantity y unit_price son obligatorios." });
  }
  if (quantity <= 0) {
  ...
  try {
    ...
    if (cart.length === 0) {
      ...
      await db.query("INSERT INTO shopping_cart (id, user_id) VALUES (?, ?)", [
        cartId,
        userId,
      ]);
    } else {
 ...
 * Actualiza parcialmente la cantidad de un ítem en el carrito usando PATCH.
 */
exports.updateCartItem = async (req, res) => {
  ...
  // ASEGÚRATE DE QUE ESTA LÍNEA ESTÉ EXACTAMENTE IGUAL A LAS OTRAS FUNCIONES:
  const userId = req.user.id;
  ...
  try {
    // 1. Buscamos el carrito que le pertenece a ESTE usuario
    const [cart] = await db.query(
      "SELECT id FROM shopping_cart WHERE user_id = ?",
      [userId],
    );
    ...
    // 2. Modificamos sólamente la fila que coincida con el carrito del usuario Y el producto enviado
    ...
    res.json({ message: "Cantidad actualizada correctamente con PATCH." });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
